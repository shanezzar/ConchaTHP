...
public class MyProgressView: UIView {
   ...
   // MARK: - Singleton
    ...
    //
    public static let shared = MyProgressView(frame: CGRect.zero)
    ...
    private let outerCircleDefaultColor = UIColor.black.cgColor
    ...
    private static func containerView() -> UIView? {
        ...
        #else
            if #available(iOS 13, *) {
                return customSuperview ?? UIWindow.keyWindow
            } else {
    ...
    @discardableResult
    public class func show(_ title: String = "Please wait...", animated: Bool = true) -> MyProgressView {
        let spinner = MyProgressView.shared
        ...
        if spinner.superview == nil {
            ...
            UIView.animate(withDuration: MyProgressView.standardAnimationDuration, delay: 0.0, options: .curveEaseOut, animations: {
                ...
                NotificationCenter.default.addObserver(
                    spinner,
                    selector: #selector(MyProgressView.orientationChangedAction),
            ...
            show(delay: MyProgressView.standardAnimationDuration, title: title, animated: true)
        }
        spinner.title = title
    ...
    /// - Returns: The instance of the spinner
    @discardableResult
    public class func show(duration: Double, title: String, animated: Bool = true, completion: (() -> ())? = nil) -> MyProgressView {
        let spinner = MyProgressView.show(title, animated: animated)
    ...
    public class func show(delay: Double, title: String, animated: Bool = true) {
        ...
        MyProgressView.shared.delay(delay, completion: {
            if let index = delayedTokens.firstIndex(of: token) {
                delayedTokens.remove(at: index)
                MyProgressView.show(title, animated: animated)
            }
        })
    }
    ...
    /// - Returns: The instance of the spinner
    @discardableResult
    public class func show(progress: Double, title: String) -> MyProgressView {
        let spinner = MyProgressView.show(title, animated: false)
    ...
    private let defaultTitleFont = UIFont(name: CustomFont.sofiaProMedium, size: 14)!
    ...
    @objc func orientationChangedAction() {
        if let _ = MyProgressView.containerView() {
            MyProgressView.shared.setNeedsLayout()
        }
    }
    
    public func updateFrame() {
        if let containerView = MyProgressView.containerView() {
            MyProgressView.shared.frame = containerView.bounds
            containerView.bringSubviewToFront(MyProgressView.shared)
        }
    }
